    ...
    try: 
        ...
        if not isinstance(precision, int):
            if not isinstance(e, TypeError):
                print(f"Raised wrong error with input of type: ({type(precision)}). Expected TypeError, got: {type(e)}", file=sys.stderr) 
                sys.exit(1)
            else: 
                sys.exit(0)
        if not precision in (32, 64):
            if not isinstance(e, ValueError):
                print(f"Raised wrong error with input of value: ({precision}). Expected ValueError, got: {type(e)}", file=sys.stderr) 
                sys.exit(1)
            else:
                sys.exit(0)
        raise e 
