...
def test_op_init_default_success():
    ...
    for name, dtype in zip(
        ...
        assert typing.array_dtype_is(array, dtype)
        assert array.ndim == 1
        assert array.size == problem.ndim

    assert typing.is_numeric(problem.known_optimum_value)
...
def test_op_broadcast(func, points):
    ...
    assert len(result) == 2
    assert typing.array_dtype_is(result, np.ndarray)
    for res in result:
        assert typing.array_dtype_is(res, "numeric")
        assert res.shape[0] == points.shape[0]
        assert res.ndim == 1

    for i in range(len(points)):
        assert result[0][i] == func(points[i])
        assert result[1][i] == 0.0
