...
class MultiConvergence(Convergence):
    ...
    def __init__(self, criteria: list[Convergence], how: str = "or"):
...
class Timeout(Convergence):
    ...
    def __init__(
        ...
        assert how in (
            "dt",
            "perf",
        ), f"`how` should be 'dt' (datetime.datetime.now) or 'perf' (time.perf_counter). Supplied {how}"
    ...
    def __call__(
        ...
        if self.attribute is not None:
            if self.attribute_in_population:
                value = (
                    getattr(intermediate_result.population, self.attribute)
                    if self.attribute is not None
                    else intermediate_result
                )
            else:
                value = (
                    getattr(intermediate_result, self.attribute) if self.attribute is not None else intermediate_result
                )
        else:
