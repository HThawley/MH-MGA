...
@njit
def mutate_gaussian_niche_float(niche, mutation_sigma, mutation_prob, rng, startidx=0):
    """
    Mutate individuals in a niche
    Compatible only with float-only
    """
...
@njit
def _mutate_int(item, mutation_sigma, rng):
    """Integer mutation for single variable. Returns integer-valued float"""
    return round(rng.normal(item, mutation_sigma))
